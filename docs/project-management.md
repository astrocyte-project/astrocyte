...
The `astrocyte-project` org defines five issue types. Pick exactly one per issue:
...
- **`component:*`** — which part of the codebase (`api`, `cli`, `core`, `agent`,
  `mcp`, `rvc`, `ha`, `web`, `docker`, `ci`, `docs`, `rag`, `backup`,
  `network`). On PRs these are applied **automatically** by the
  [labeler](../.github/labeler.yml) based on the files changed; on issues
  they're set during triage.
- **`phase-0..3`** — the roadmap phase a piece of work belongs to (thematic).
- **Meta** — `epic`, `backlog`, `needs-triage`, `blocked`, `rv-deployment`
  (the RV reference-deployment initiative, ADR-010), plus GitHub defaults
...
| **v0.3 — RV coach node: telemetry & safe control** | `rv-deployment` | RV-C bridge, HA integration, actuation policy, multi-node deploy ([ADR-010](adr/ADR-010-rv-reference-deployment.md)) |
| **v0.4 — Ops Agent & curated apps** | phase-1 | Ops Agent, `aios` CLI, app registry, zero-trust |
| **v0.5 — RAG layer & unified search** | phase-2 | ChromaDB, ingestion, connectors, search |
| **v1.0 — Atomic backup & polish** | phase-3 | Backup/restore, AI-verified recovery, RV reference-deployment validation, polish |

The v0.3/v0.4 milestones were re-sequenced by
[ADR-010](adr/ADR-010-rv-reference-deployment.md), which made the RV coach the
1.0 reference deployment; renames are expressed declaratively via `from:` in
`.github/project.yml` and applied in place (issue associations preserved).

## The Project board

Work is tracked on the org Project board **[Astrocyte 1.0](https://github.com/orgs/astrocyte-project/projects/11)**
